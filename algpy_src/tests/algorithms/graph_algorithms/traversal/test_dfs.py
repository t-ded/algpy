...
@pytest.fixture
def dfs() -> DepthFirstSearch:
    return DepthFirstSearch()


def test_dfs_base(dfs: DepthFirstSearch) -> None:
    assert dfs.name == 'Depth First Search'
    assert dfs.best_case_time_complexity == '1'
    assert dfs.best_case_description == 'starting from searched for element'
    assert dfs.average_case_time_complexity == '|V| + |E|'
    assert dfs.worst_case_time_complexity == '|V| + |E|'
    assert dfs.worst_case_description == 'searched for element not present in the graph'
    assert dfs.space_complexity == '|V|'
    assert dfs.get_worst_case_arguments(GraphSize(*(5, 5))) == {
        'input_instance': Graph({0: {1: None, 2: None, 3: None, 4: None}, 1: {0: None, 2: None}, 2: {0: None, 1: None}, 3: {0: None}, 4: {0: None}}),
        'element_to_search': 6
    }


def test_worst_case(dfs: DepthFirstSearch) -> None:
    assert dfs.n_ops == 0
    worst_case_args = dfs.get_worst_case_arguments(GraphSize(*(5, 5)))
    expected_traversal_order: list[int] = [0, 4, 3, 2, 1]
    expected_traversal_graph = TraversalGraph()
    expected_traversal_graph.add_nodes_from(expected_traversal_order)
    assert dfs.run_algorithm(**worst_case_args) == (
        False,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param({1: {2: None, 4: None, 6: None}, 2: {3: None}, 3: {}, 4: {5: None}, 5: {}, 6: {7: None}, 7: {}}, 8, [1, 6, 7, 4, 5, 2, 3], 14, id='Star graph with two layers'),
    ]
)
def test_dfs_run_algorithm(
        dfs: DepthFirstSearch, input_adjacency_list: dict[Node, dict[Node, SingleEdgeData]], element_to_search: Node,
    ...
    assert dfs.run_algorithm(graph, element_to_search=element_to_search) == (expected_verdict, expected_traversal_graph)
    assert dfs.n_ops == expected_n_ops
    assert dfs.run_algorithm(digraph, element_to_search=element_to_search) == (expected_verdict, expected_traversal_graph)
    assert dfs.n_ops == expected_n_ops
